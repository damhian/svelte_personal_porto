<script lang="ts">
	...
	const handleMouseMove = (e: MouseEvent) => {
		...
		const speed = Math.sqrt(Math.pow(mousePos.x - lastMoustPos.x, 2));
		...
		gsap.to('.hover-reveal', {
			...
			rotation: speed * (mousePos.x > lastMoustPos.x ? 1 : -1),
			ease: 'back.out(2)',
			duration: 1.3
		});
	...
	{#each items as post, index (post.id + index)}
		...
		>
			...
			>
				...
				<span class="ml-auto flex items-center gap-2 font-medium md:ml-0"
					>{viewMoreText}
					<IconArrow />
				</span>
			</PrismicLink>
